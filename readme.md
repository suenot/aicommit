...
flowchart TD
    ...
    %% Режим отслеживания
    K --> K1[Настройка интервала отслеживания]
    K1 --> K2[Цикл наблюдения]
    K2 --> K3{Есть изменения?}
    K3 -->|Да| K4{Прошло время после редактирования?}
    K3 -->|Нет| K2
    K4 -->|Да| K5{Прошло время с последнего коммита?}
    K4 -->|Нет| K2
    K5 -->|Да| J
    K5 -->|Нет| K2
